...
node *add(node *avl, int data){
    ...
    // avl node 'nin sag ve sol kismindan gelen en yuksek deger o node nin yuksegligi olur.
...
node *delete(node *avl, int data){
    ...
    // sag ve sol agacın en yuksek degeri alinir ve atanir.
    avl->height = ((left0>right0) ? left0 : right0) + 1;
...
void Inorder(node *tree){
    ...
    Inorder(tree->left);
    printf("(data = %d,height = %d) ",tree->data, tree->height);    
    Inorder(tree->right);
